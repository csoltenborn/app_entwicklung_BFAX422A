...
Auch wenn es ein langer Weg war, meine Erweiterungen wie gewünscht umzusetzen, so bin ich doch stolz meine fertige und erweiterte Android-Applikation präsentieren zu können. Es wurden alle Probleme, die Anfangs definiert wurden, aufgelöst oder umgewandelt und dann gelöst. So ist es dem Nutzer jetzt möglich (solang er einen gültigen API-Schlüssel besitzt) über die App mehrere Konversationen mit ChatGPT zu führen. Diese werden dann über die Laufzeit hinweg persistent gespeichert und bei Start der App wieder geladen. Der Benutzer kann zudem, sollte sein Nachrichtenverlauf binnen einer Konversation die Höhe der Präsentationsfläche übersteigen, auch seine älteren Nachrichten nachschlagen, in dem er durch seinen Verlauf scrollt. Zudem wurde die App visuell deutlich aufpoliert. Im Hintergrund befindet sich nun ein Hintergrundbild, was das Auftreten der App dynamischer gestaltet, sowie den Text besser vom Hintergrund abhebt. Zudem wurden die bestehenden sowie die neuen Schaltflächen um grafisch passende Symbole und Hintergründe ergänzt, sodass ein zusammenpassender Gesamteindruck der App überzeugt. Zu guter Letzt hat auch die Implementation der neuen Fehlerbox und der neuen Pause-Schaltfläche gut funktioniert. Tritt kein Fehler auf, so merkt der Benutzer nicht, dass es eine Fehlerbox gibt. Tritt doch ein Fehler auf, so wird der Benutzer nicht mit dem Absturz der App konfrontiert, sondern sieht, dass scheinbar irgendetwas nicht ganz so funktioniert hat, wie es sollte. Dies wird zudem durch eine rote Schriftfarbe der Fehlerbox verdeutlicht.

### Persönliches Fazit
Auch wenn ich viel im Rahmen dieses App-Projekts gelernt habe, so war es doch zeitlich schwierig alles unter einen Hut zu bekommen. Neben recht hohem Lernaufwand für Prüfungen, Präsentationen in beinahe jedem Fach und zusätzlichem Aufwand für Recherchen und der Einarbeitung in das Studium, hatte ich den Zeitaufwand deutlich unterschätzt. Vor allem die abschließende Dokumentation meiner Erweiterung war zeitintensiver als gedacht.
Würde ich das Projekt weiterempfehlen? Auf jeden Fall, Spaß gemacht hat es.
