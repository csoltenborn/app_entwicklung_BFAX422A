...
Auch wenn es ein langer Weg war, meine Erweiterungen wie Gewünscht umzusetzen, so bin ich doch stolz meine fertige und erweiterte Android-Applikation präsentieren zu können. Es wurden alle Probleme, die Anfangs definiert wurden, aufgelöst oder umgewandelt und dann gelöst. So ist es dem Nutzer jetzt möglich (solang er einen entsprechenden API-Schlüssel besitzt) über die App mehrere Konversationen mit ChatGPT zu führen. Diese werden dann über die Laufzeit hinweg persistent gespeichert und bei Start wieder geladen. Der Benutzer kann zudem, sollte sein Nachrichtenverlauf binnen einer Konversation die höhe der Präsentationsfläche übersteigen, auch seine älteren Nachrichten nachschlagen, in dem er durch seinen Verlauf scrollt. Zudem wurde die App visuell deutlich aufpoliert. Im Hintergrund befindet sich nun ein Hintergrundbild, was das auftreten der App dynamischer gestaltet, sowie den Text besser vom Hintergrund abhebt. Zudem wurden die bestehenden sowie die neuen Schaltflächen um grafisch passende Symbole und Hintergründe ergänzt, sodass ein zusammenpassendes Gesamtauftreten der App überzeugt. Zu guter letzt hat auch die Implementation der neuen Fehlerbox und der neuen Pause-Schaltfläche gut funktioniert. Tritt kein Fehler auf, so merkt der Benutzer nicht, dass es eine Fehlerbox gibt. Tritt doch ein Fehler auf, so wird der Benutzer nicht mit dem Absturz der App konfrontiert, sondern sieht, dass scheinbar irgendetwas nicht ganz so funktioniert hat, wie es soll. Das wird durch eine rote Schriftfarbe der Fehlerbox verdeutlicht.

### Persönliches Fazit
Auch wenn ich viel im Rahmen dieses App-Projekts gelernt habe, so war es doch zeitlich schwierig alles unter einen Hut zu bekommen. Neben recht hohem Lernaufwand für Prüfungen, Präsentationen in beinahe jedem Fach und zusätzlichem Aufwand für Recherchen und der Einarbeitung in das Studium, so entsteht viel Druck und die persönliche Freizeitgestaltung läuft gegen Null.
Würde ich das Projekt weiterempfehlen? Auf jeden Fall, spaß gemacht hat es! Nur der Gesamtkontext und der Zeitraum, in welchem die App umgesetzt werden sollte, war unglücklich gewählt.
